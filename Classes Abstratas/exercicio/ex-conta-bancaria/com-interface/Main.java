	...
	private static void mostrarSaldo(Scanner input, ArrayList<ContaBancaria> contas) {
		...
		if(conta instanceof ContaPoupanca)
			...
			System.out.printf("Cliente: %s , Conta: %s. (Investimento)%n"
					+ "Saldo da conta: R$%s.%n", ((ContaInvestimento)conta).getCliente(), 
												 ((ContaInvestimento)conta).getNumeroDaConta(), 
												 ((ContaInvestimento)conta).getSaldo());
		
	}

	private static void calcularTributos(Scanner input, ArrayList<ContaBancaria> contas) {
		System.out.println("Numero da conta:");
		String numeroDaConta = input.next();
		...
		BigDecimal tributos = 
				((ContaInvestimento)conta).calcularTributos(BigDecimal.valueOf(taxaDeRendimento));
